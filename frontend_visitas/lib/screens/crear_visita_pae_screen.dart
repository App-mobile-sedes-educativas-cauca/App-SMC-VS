...
class CrearVisitaPAEScreen extends StatefulWidget {
  const CrearVisitaPAEScreen({super.key});
...
class _CrearVisitaPAEScreenState extends State<CrearVisitaPAEScreen> {
  ...
  Future<void> _guardarVisita() async {
    ...
    final itemsPAE = await _futureItemsPAE;
    final itemsFaltantes = itemsPAE.where((item) => !_evaluaciones.containsKey(item.id)).toList();
    
    if (itemsFaltantes.isNotEmpty) {
      _mostrarError('Por favor complete todas las evaluaciones');
      return;
    }
    ...
    try {
      // Crear lista de evaluaciones
      final evaluaciones = _evaluaciones.entries.map((entry) {
        return EvaluacionItem(
          visitaId: 0, // Se asignará en el backend
          item: itemsPAE.firstWhere((item) => item.id == entry.key).nombre,
          valor: entry.value,
        );
      }).toList();
      ...
      final resultado = await _apiService.guardarVisitaConEvaluacionesPAE(
      ...
      if (resultado) {
        _mostrarExito('Visita PAE guardada exitosamente');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Crear Visita PAE 2025'),
        backgroundColor: Colors.blue,
        foregroundColor: Colors.white,
        actions: [
          IconButton(
            icon: const Icon(Icons.list),
            onPressed: () => Navigator.pushNamed(context, '/cronogramas_guardados'),
            tooltip: 'Ver Cronogramas Guardados',
          ),
        ],
      ),
      body: _isLoading
                            ...
                            children: [
                              ...
                              DropdownButtonFormField<String>(
                                ...
                                onChanged: (String? valor) {
                                  _actualizarEvaluacion(item.id, valor ?? '');
                                },
                              ),
                              const Divider(),
                            ],
